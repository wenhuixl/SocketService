...
function SetOfflineParam(ServerIP,ServerPort,ParkID,isRecordCover,parkInOutFlag,MonthcarAlarmDays,RecognitionAccuracy,RecordMatchAccuracy,MonthCarToTempcarFlag,MonthCarOpenType,TempCarOpenType,MinCharge,TempCarForbiddenFlag,SyncTimeFromMaster,OnlineFlag,OneChannelMode,OneChannelWaitTime,NormalModeWaitTime,MinChargeFlag,DisplayRefreshInterval,PropertyLogo,ScreenType) {    //1244
    ...
    var head = Buffer.from('dc040000749f8ad3a1000000','hex');
    var data = Buffer.alloc(149);
    ...
    data.writeIntLE(SyncTimeFromMaster,37,4);      /**< 时间点23:00-->2300 -->0x08fc */
    data.writeIntLE(OnlineFlag,41,1);              /**< 是否在线模式，0：脱机，1：在线, 参考枚举ONLINE_MODE_E */
    data.writeIntLE(OneChannelMode,42,1);          /**< 是否启用单通道模式，0：否，1：是, 参考枚举ENABLED_TYPE_E， 0： 不启用单通道，1： 单通道模式*/
    data.writeIntLE(OneChannelWaitTime,43,4);      /**< 单通道重复车牌等待时间， 以秒为单位 */
    data.writeIntLE(NormalModeWaitTime,47,4);      /**< 正常模式重复车牌等待时间， 以秒为单位 */
    data.writeIntLE(MinChargeFlag,51,1);           /**< 是否启用最低收费， 0： 不启用， 1： 启用, 参考枚举ENABLED_TYPE_E， */
    data.writeIntLE(DisplayRefreshInterval,52,1);  /**< 脱机显示屏显示内容的刷新间隔， 以秒为单位 */
    data.write(PropertyLogo,53,64);           /**< 脱机显示屏默认显示的企业标识 */
    data.writeIntLE(ScreenType,117,1);             /**< 显示屏类型， 参考枚举ScreenType */
        //保留字段长度：31
    var result = Buffer.concat([head,data],161);
    return result;
}
...
function GetDeviceDataCount(DataType,IDFilter) {     //1248
    ...
    // }FCT_TYPE_E;
    data.writeIntLE(IDFilter,4,4);      //ID值，过滤数据用， 只能取到大于此ID值的数据，只有当数据类型为5、 7时， ID值才有效
...
function GetDeviceData(DataType,IDFilter,Offset,DataNum) {      //1250  待补充
    ...
    // }FCT_TYPE_E;
    data.writeIntLE(IDFilter,4,4);      //ID值，过滤数据用， 只能取到大于此ID值的数据，只有当数据类型为5、 7时， ID值才有效
    data.writeIntLE(Offset,8,4);       //读取偏移量。 为防止一次未读完的情况， 设置当前要读的偏移量， 以便新偏移量重新发送请求
    data.writeIntLE(DataNum,12,4);      //读取数据个数
...
function ClearDeviceData(DataType) {      //1255
